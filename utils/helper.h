...
namespace PgHelpers {
    ...
    inline uint64_t pack_case_flags_naive_8(char* ptr) {
        uint64_t flags = 0;
        uint64_t tmp;
        char* ptr_cpy = ptr;
        memcpy(&tmp, ptr_cpy, 8);
        flags |= ((tmp & 0x2020202020202020) >> 5);
        ptr_cpy += 8;

        memcpy(&tmp, ptr_cpy, 8);
        flags |= ((tmp & 0x2020202020202020) >> 4);
        ptr_cpy += 8;

        memcpy(&tmp, ptr_cpy, 8);
        flags |= ((tmp & 0x2020202020202020) >> 3);
        ptr_cpy += 8;

        memcpy(&tmp, ptr_cpy, 8);
        flags |= ((tmp & 0x2020202020202020) >> 2);
        ptr_cpy += 8;

        memcpy(&tmp, ptr_cpy, 8);
        flags |= ((tmp & 0x2020202020202020) >> 1);
        ptr_cpy += 8;

        memcpy(&tmp, ptr_cpy, 8);
        flags |= ((tmp & 0x2020202020202020)     );
        ptr_cpy += 8;
    ...
    inline void convert_to_proper_case(uint8_t* case_mask_stream, char* block, int64_t length) {
        int64_t j = 0, i = 0;
        uint64_t* block_64 = (uint64_t*)block;
        uint64_t* case_mask_stream_64 = (uint64_t*)case_mask_stream;

        uint64_t flags;
        while (j < length / 8) {
            memcpy(&flags, case_mask_stream_64 + i++, 8);
            block_64[j++] |= ((flags & 0x0101010101010101) << 5);
            block_64[j++] |= ((flags & 0x0202020202020202) << 4);
            block_64[j++] |= ((flags & 0x0404040404040404) << 3);
            block_64[j++] |= ((flags & 0x0808080808080808) << 2);
            block_64[j++] |= ((flags & 0x1010101010101010) << 1);
            block_64[j++] |= ((flags & 0x2020202020202020)     );
            block_64[j++] |= ((flags & 0x4040404040404040) >> 1);
            block_64[j++] |= ((flags & 0x8080808080808080) >> 2);
        }
    }
