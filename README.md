...
[![Bioconda downloads](https://img.shields.io/conda/dn/bioconda/ffc.svg?style=flag&label=Bioconda%20downloads)](https://anaconda.org/bioconda/ffc)

Fast FASTA Compressor (FFC) is a tool designed for compressing FASTA files. It is focused on speed and for non-redundant (or mildly redundant) DNA sequences it basically packs fours of nucleotides into bytes. If, however, data seem compressible, like for e.g. virus strains, it makes use of a fast zstd mode. We point, however, that FFC accepts any input, and for example its performance on a tar genomic archive is essentially not worse than on its raw (FASTA) contents (and if the genomes within the tar are similar, and small enough to fit a block, may even be better due to exploited repetitions). FFC compression and decompression speed is often around 1500-2000 MB/s on an SSD, and above 3000 MB/s in RAM memory.
...
To overwrite an existing output file, use:
```shell
-f or --force
```
...
The backend compression is based on zstd and by default it detects if zstd compression is needed for the actual data (we call this mode "adaptive"). Alternatively, you can specify the compression level as 
-l or --level
